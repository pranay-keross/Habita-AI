...
- **A per-module permission matrix on Family.** The real backend only has `OWNER`/`ADMIN`/`MEMBER` roles — the old local model's `permissions: {medicines, expenses, documents, safety}` toggle grid was deleted, not kept as fake state (`docs/DECISIONS.md` D-023). Gating is role-only now: `MEMBER` read-only, `ADMIN`/`OWNER` full access.
- **Reliable "which family member is me."** `FamilyMemberResponse` has no `userId` for a non-owner row — `resolveMyMembership()` (`src/features/family/api.ts`) falls back to a best-effort name match, defaulting to the least-privileged read if it can't resolve. A real limitation of the backend contract, not a client bug — see `docs/ARCHITECTURE.md` §7 and `docs/BACKEND_CONTEXT.md`.
- **Self-service "leave family" for a plain `MEMBER`.** The backend's Remove Member endpoint needs admin access on the *caller*; only a resolved `ADMIN` can leave via this API today.
