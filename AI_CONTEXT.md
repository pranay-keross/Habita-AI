...
>
> **Last synced:** 2026-08-11 · branch `initial-static` · after the **Saheli → Habita AI rebrand** (D-019, D-020), `M2-T2` (resend timer), `M4-T1/T2/T3/T5` (Medical Chest), **real Family & Managed Members backend integration** (D-023) — superseding `M3`'s local-only Family model with a live `/api/families/**` contract — a **Postman-collection reconciliation pass** (D-024): added the collection's `GET /invites/history` endpoint (client + admin UI, all 6 locales) and corrected every doc reference to the collection's actual on-disk filename, dashboard/Family UX polish (D-025, D-026), **silent session-token refresh** (D-027) — `useAuth()` now calls the long-existing `refresh()` on an expired token instead of trusting it forever, which also surfaced a real, separate, backend-side bug in `PUT /profile/details` (`docs/BACKEND_CONTEXT.md` §3) — **account-scoped local data no longer leaking across sign-in/sign-out** (D-028): signing out (or a new phone number signing in on a device that still holds another account's cache) now clears `habita.user_profile`/`habita.medicines`/`habita.medicine_intake_log`, and a local-cache-vs-live-fetch race in Profile edit that could silently re-clobber fresh backend data with stale cache is fixed — and (D-029): the boot guard now actually validates/refreshes the stored session instead of just checking it exists (a long-idle reopen with a truly dead token now correctly routes to sign-in instead of stranding the user on a broken Dashboard/Profile), Profile edit now sources the phone field from the session (not an often-empty local cache) and applies the account's saved `preferredLanguage` from the backend, and **Dashboard now does the same photo/language sync on every focus** — not just Profile edit — so a returning user sees their own photo and language the moment they land on Dashboard · `npx tsc --noEmit`, `npm run lint` (zero warnings) and `npm test` (34 tests) all pass.
>
> **2026-08-18 addendum (not a full re-sync — see `docs/DECISIONS.md` D-031–D-038 for what happened in between):** **D-039** collapsed Family's role model from `OWNER`/`ADMIN`/`MEMBER`/`VIEWER` to `OWNER` (creator) / `MEMBER` — every member can now add (invite, managed-member), only the creator can remove another member, and any non-creator member can leave on their own; fixed a real security hole where Medicine's `DELETE /api/medicine/{id}` had no access check at all and wired it up client-side; and closed three gaps in the already-built prescription-upload → AI-extraction pipeline (`ocrStatus` now genuinely transitions instead of sitting at `PENDING` forever, a duplicate medicine name during extraction no longer rolls back the whole upload, and the client now shows what got auto-added).
