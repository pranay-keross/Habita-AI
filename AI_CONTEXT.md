...
>
> **Last synced:** 2026-08-22 · branch `initial-static` · after the **Saheli → Habita AI rebrand** (D-019, D-020), `M2-T2` (resend timer), `M4-T1/T2/T3/T5` (Medical Chest), **real Family & Managed Members backend integration** (D-023) — superseding `M3`'s local-only Family model with a live `/api/families/**` contract — a **Postman-collection reconciliation pass** (D-024): added the collection's `GET /invites/history` endpoint (client + admin UI, all 6 locales) and corrected every doc reference to the collection's actual on-disk filename, dashboard/Family UX polish (D-025, D-026), **silent session-token refresh** (D-027), **account-scoped local data no longer leaking across sign-in/sign-out** (D-028), session validation on boot & photo/language sync (D-029), **family relation tracking** (D-030), **Medchest real API integration** (D-032, D-035, D-038), **Prescription document picker & OCR** (D-037, D-040, D-041, D-042), and **Medicine payload normalization & error handling** (D-043) · `npx tsc --noEmit`, `npm run lint` (zero warnings) and `npm test` (35 tests) all pass.
>
> **2026-08-22 addendum:** **D-043** resolved medicine creation and OCR quantity confirmation failures against `Saheli Backend — Auth, Profile & Family.postman_collection.json`: handles `lowStockThreshold: null` on OCR medicines without 400s on update, preserves slot keys when `scheduleTimes` is returned as a map/object, sanitizes adherence rate parsing, extracts detailed Spring Boot validation errors to user alerts, and provides a clear profile creation empty state in `MedicineScreen` when a user has a family but no profiles yet.
