...
With hard evidence that the plain-string path is _definitely_ wrong, and no equivalent evidence that the object-with-`type` path is wrong — only a plausible-sounding but unconfirmed theory — the balance of evidence flipped.
...
**Decision.** Do not attempt a client-side workaround. There is no way for `profile.tsx` to detect "this user already has a profile" before calling `create` — that information doesn't exist on the client (`GET /profile/details` failing with a 500 is currently the _only_ signal a profile doesn't exist yet, per the collection's documented "500 — profile not created yet" example, which is itself the same class of bug: a missing-row case surfacing as an unhandled exception instead of a clean 404/empty response). Fixing this requires backend changes (idempotent create, or a clean conflict/not-found response) that are out of this repository's scope.
...
- `Saheli-Backend.postman_collection.json` (repo root) — this names the _external_ backend service's own exported artifact, not this repo's branding; this repo doesn't control that service's name.
...
- Historical entries earlier in this decision log (e.g. D-006, D-007, D-013–D-018) — they describe what was true _at the time each decision was made_, including the old `saheli.*` keys and brand name; append-only means this entry documents the change, not a rewrite of that history.
...
**Decision.** `docs/BACKLOG.md`'s M4–M8 were restructured around the SRS's actual module groups instead of the old generic "Medicine/Documents/Money/Safety" grouping (milestone _numbers_ M4–M7 kept stable so existing prompt cross-references stay valid; their _content_ changed):
...
1. **The family member with `relation === 'self'` is treated as the current device's user.** `FamilyScreen.tsx`'s "Leave Family" action and `MedicineScreen.tsx`'s permission gate (`role !== 'viewer' && permissions.medicines`) both key off this member. It is explicitly _not_ a real identity system — there is no login-to-member linkage, no way for a second device to represent a different member as "self." It is documented in both call sites as a stand-in for what `M8`'s real multi-tenant backend will need to do properly (a session belongs to a specific member, not to "the app").
...
**Context.** User report: editing the profile name and tapping Save showed "Couldn't save your profile. Try again." Two things ruled out first, by reading the code: (1) `name` was never the cause — `PUT /profile/details`'s DTO has no `name` field at all (`docs/DECISIONS.md` D-013), so a changed name is never even sent on this path; (2) this wasn't reproducible from source alone, since `profile.tsx`'s error handling only logs `console.warn('Profile save failed:', err)` and shows a generic localized alert — the _real_ error was never visible without live testing.
...
**Consequences.** `npx tsc --noEmit`, `npm run lint`, `npm test` (34/34) all still pass — this change touches no UI, no locale strings, no new dependency. The client-side symptom (silent, unexplained save failures after roughly an hour of app use) is fixed: sessions now self-heal via silent refresh instead of accumulating stale tokens that fail every write. Editing a profile's email/language/city still cannot be confirmed working end-to-end, because the newly-exposed backend bug now blocks it unconditionally — this is a regression in _what's provable_, not a regression in the client, and is the natural next thing for whoever owns the Spring Boot backend to fix (`docs/BACKEND_CONTEXT.md` §5's suggested work order should be read as superseded on this one point: fixing `PUT /profile/details` is now higher priority than it looked before this session, since it's not a hypothetical gap anymore but a confirmed blocker). `docs/BACKLOG.md` `M2-T3` should be read as further along after this — refresh is wired and confirmed live — but still not fully done, since idle/absolute expiry _detection_ (as opposed to reactive refresh-on-use) and `M2-T5`'s re-auth banner for a truly dead refresh token remain unbuilt.
...
2. **A separate race inside `profile.tsx`'s edit-mode load**: the local-cache read (`getItem(PROFILE_STORAGE_KEY, ...)`) and the live `GET /profile/details` fetch are two independent, unordered async operations, each unconditionally calling `setName`/`setEmail`/`setLocation` on resolution. The code's own comment already stated the intended contract — "local storage above is the immediate/offline fallback... the backend is the authoritative record once reachable" — but nothing enforced it: if the local-cache callback happened to resolve _after_ a successful live fetch, it would silently overwrite fresh backend data with stale cached data. Observed directly during this investigation: a screen showed the correct backend name paired with a stale, previously-typed test email — the live fetch had already won for `name` by the time it ran, but the cache callback ran a moment later and re-clobbered `email`.

**Decision.** Two changes:

- `src/hooks/useAuth.ts` gained `clearAccountData()`, wiping `habita.user_profile`, `habita.medicines`, and `habita.medicine_intake_log` (deliberately **not** `habita.user_phone` — `onboarding/phone.tsx` always overwrites it with the newly-entered number _before_ this can run, so clearing it here would erase the value the very next screen needs; device preferences `habita.lang`/`habita.theme.palette` are untouched, since they aren't account data). Called from two places: `logout()` (so an explicit sign-out actually leaves a clean slate, closing the gap `M2-T6` left), and `verify()` — pre-emptively, before persisting the new session, comparing the _cached profile's own phone_ (not the old session's, since that's what actually survives an incomplete sign-out) against the newly-verified phone via a shared trailing-10-digit normalization; a mismatch triggers the clear. Concurrent-refresh-style module-level state wasn't needed here since this only runs once, synchronously within `verify()`.
...
1. **The boot guard (`_layout.tsx` via `useAuth()`'s mount effect) only ever checked whether a `habita.session` object exists in storage** — never whether it (or a refresh of it) actually still works. `signedIn` was `true` for a session whose access token _and_ refresh token had both died, so a long-idle reopen still landed on Dashboard with a token that would fail every request, rather than routing back to sign-in. `D-027`'s refresh logic already existed and was already used by every screen's `getAccessToken()`/`getUserId()` — it just was never consulted at boot, the one place that decides which screen the user lands on at all.
2. **The Profile edit screen never populated the phone field from anything reliable.** It only ever read `data.phone` from the local `habita.user_profile` cache — a key that only gets a `phone` value once the user has saved at least once _on this device_. A freshly signed-in account (or one whose cache was just correctly cleared by `D-028`) has no such value yet, so the field rendered empty even though the backend (`ProfileDetailsResponse.phone`) and the session itself (`Session.phone`, set at `verify()` to exactly what was typed) both had the real number the whole time.
3. **`preferredLanguage` from `GET /profile/details` was fetched and discarded** — on the Profile screen specifically, the only screen that fetched it at all. The account's saved language preference exists in the backend response but nothing ever read it — the picker (and the whole app, since language is a shared `i18n` module, not screen-local state) kept showing whatever this _device_ last had active, not what this _account_ had actually saved.
...
- `useAuth()`'s mount effect now runs the stored session through `getValidSession()` (the same function `getAccessToken()`/`getUserId()` already used, `D-027`) before setting `pending: false` — silently refreshing a merely-expired token, or clearing a genuinely-dead one, _before_ `signedIn` is computed. A session that's actually recoverable now still boots straight to Dashboard (unchanged, no added latency for the common case); a session that isn't now correctly boots to Language instead of stranding the user on a broken authenticated screen.
...
- `{{family}}`/`{{medicine}}` are still blank as _collection variables_ in the export, but every captured response's own `path` field reveals what they resolved to when the examples were actually recorded: `{{family}}` = `{{baseUrl}}/api/families`, `{{medicine}}` = `{{baseUrl}}/api/medicines`. This is the same technique that would have resolved D-024's concern at the time, just not applied then.
...
**Consequences.** `npx tsc --noEmit`, `npm run lint`, `npm test` (35/35) all pass. One new native dependency requires a fresh native build (already the case for every APK rebuild in this session) — no Android-side manual linking needed, autolinking picks it up. One new `profile.*` locale key (`delete_account_already_pending`, 23 → 24 per locale, verified equal across all six). This does not fix the _underlying_ reason a deletion might already be pending (most likely: this exact feature was tested successfully once already, earlier in this session, against a real account) — it makes that state legible instead of silently indistinguishable from a genuine failure. Not re-verified live against the deployed backend with a real account in this session.
...
- Adding this dependency broke `npm test` twice in succession, in the two ways `docs/ARCHITECTURE.md` §10 already documents as the standard failure modes for a new RN dependency: (1) it ships untranspiled ESM, so `jest.config.js`'s `transformIgnorePatterns` needed `@react-native-documents` added to the allowlist (it doesn't match the existing `@react-native(-community)?` pattern); (2) it reaches for a native TurboModule at import time, so it needed a jest mock. The package _does_ ship one, but as a `setupFiles`-style script that self-registers `jest.mock()` calls against its own deep, build-output-relative internal paths (`../src/spec/NativeDocumentPicker`, etc.) — too fragile to `require()` inline the way `@react-native-community/datetimepicker`'s mock (D-036) was. Hand-rolled a plain mock in `jest.setup.js` instead, matching the `@react-native-community/geolocation` precedent's shape exactly.
...
## D-038 — Medchest profiles can now represent any family member (or a no-account "Other"), not just the caller; Medicine screen gets a profile switcher

**Status:** Accepted · **Date:** 2026-08-17

**Context.** Direct user report: the Medicine screen let a family create only one medicine profile (auto-created for "myself," per D-035/D-036), with no way to track medicines for a spouse, child, or elderly parent, and no way to add someone who doesn't have an account or isn't in the family yet. Asked to check the collection for the real shape before building anything. It had changed again since D-032/D-035/D-037 (6771 → 6959 lines): `POST /families/{id}/profiles` split into two named requests with a real answer to exactly this —

- **"Create Profile — Existing Family Member"**: body `{familyMemberId, category, dateOfBirth}`. `familyMemberId` is any row from `Family.members[]` (`src/features/family/types.ts`) — works identically for a full account-holder member and a no-login `ManagedMember`/dependent. `name` is **derived server-side** from the linked member; the collection's own description states any `name` sent is ignored. 404 `"Family member not found in this family"` if the id doesn't belong to this family.
- **"Create Profile — Other (no account / not in family)"**: body `{name, category, dateOfBirth}`, no `familyMemberId`. For this path `name` is required and typed by the user — 400 `"name is required when familyMemberId is not given (Other)"` if omitted. The resulting profile's `familyMemberId` is `null`.
- Both `GET /families/{id}/profiles` (list) and every Create response now include `familyMemberId` (nullable) alongside the existing `id`/`name`/`category`/`dateOfBirth` fields.
- Still no delete endpoint anywhere in the domain (profile, medicine, or document) — unchanged from D-032.

**Decision.**

- `src/features/medicine/api.ts`: `FamilyProfile` gained `familyMemberId: string | null`. The old single `createFamilyProfile` split into `createFamilyProfileForMember(familyId, familyMemberId, category, dateOfBirth, token)` and `createFamilyProfileOther(familyId, name, category, dateOfBirth, token)`, mirroring the collection's own two request names exactly (the established pattern in this codebase — Family's `acceptInvite`/`declineInvite` split the same way). `parseMedchestError` gained `"Family member not found in this family"` → `not_found`.
- `MedicineScreen.tsx` no longer auto-creates a single `SELF` profile. It now fetches the family's full profile list (`listFamilyProfiles`) and renders a horizontal **profile switcher** — a chip per profile (name + a category icon: 🧑 self, 🧒 kid, 👵 elder, 👤 other), defaulting to a `SELF` profile if one exists, else the first profile, else none (a "no profiles yet" empty state with its own "Create Profile" button). Tapping a chip re-fetches that profile's medicines and documents; a trailing "+" chip opens the new **two-step "Add Profile" sheet**: step one lists every `family.members[]` row (with a "Dependent" tag for managed members) plus a distinct "Someone Else" row for the no-account/not-in-family case; step two asks for category (chip picker: Myself/Child/Elder/Other) and date of birth (reusing D-036's native calendar picker), plus a name field **only** when "Someone Else" was picked — picking an existing member never asks for a name, since the backend derives it. Picking yourself pre-selects the `SELF` category as a default, still changeable.
- Removed the D-035/D-036 flow this replaces (`showDobSheet`/`handleConfirmDob`, the implicit "tap + Add with no profile → prompted for just a DOB → a SELF profile silently appears") along with its now-dead locale keys (`dob_prompt_title`, `dob_prompt_helper`, `save_dob`, `dob_invalid`, `default_profile_name`) — 22 new keys added in their place (net +17 per locale, 53 → 70, verified equal across all six) covering the switcher, the two-step sheet, and the category labels.
- "Add Profile" is reachable regardless of `canEdit` (the existing Family-admin-derived permission flag that already gates editing medicines) — the collection's own description states this endpoint accepts `OWNER`/`ADMIN`/**`MEMBER`** write access, a deliberately wider grant than Family's own member-management actions. Medicine CRUD/intake/documents keep the existing `canEdit` gate unchanged, since nothing in this pass contradicted that.

**Consequences.** `npx tsc --noEmit`, `npm run lint`, `npm test` (35/35) all pass. Not verified live — same caveat as every Medchest change this session, read cold from the collection's saved examples. Specifically unconfirmed: whether creating a second profile for the _same_ `familyMemberId` is rejected or silently allowed (untested, no example either way); whether category truly stays restricted to `SELF`/`KID`/`ELDER`/`OTHER` now that the "Existing Family Member" request's own description calls it "free text" (contradicting the still-present `CHILD`→500 test case in the same collection — the enum-restricted reading was kept, since it's backed by an actual request/response pair, not just prose). `docs/BACKLOG.md`'s `M4-T10` and `docs/BACKEND_CONTEXT.md`'s Medchest subsection updated to match.

---

## D-039 — Family role model collapsed to creator/member; Medicine's unsecured delete endpoint fixed and wired up; prescription-upload gaps closed

**Status:** Accepted · **Date:** 2026-08-18

**Context.** Direct user report, on the real backend (`C:\Backend\Saheli-Backend`, this repo's `src/config.ts` currently points at a local instance of it, `http://10.0.2.2:8080/api`): inviting a family member surfaced them as "view only," which wasn't wanted at all — the user does not want an `OWNER`/`ADMIN`/`MEMBER`/`VIEWER` hierarchy, just a single distinction between whoever created the family and everyone else, with a specific rule set: any member can add (invite/managed-member), only the creator can remove another member, and any non-creator member can remove themselves (leave). Separately, the user asked for the "remaining medicine APIs" — reading the backend source (not just the collection) found the prescription-upload → AI-extraction → auto-create-medicines pipeline already fully built and already called from the app, so the actual work was closing three real gaps in it rather than building it from scratch, plus a genuine security bug found in the same pass: `MedicineController.deleteMedicine` → `MedicineService.deleteMedicine` called `medicineRepository.deleteById` directly, with **no `familyAccessService` check at all**, unlike every sibling method in that class — any authenticated user could delete any medicine in the system by id, cross-family. The frontend never called this endpoint either, with a comment claiming no delete endpoint existed; it did, just unsecured.

**Decision.**

- **Backend, `family` package** (`C:\Backend\Saheli-Backend`): `FamilyRole` enum shrunk from `{OWNER, ADMIN, MEMBER, VIEWER}` to `{OWNER, MEMBER}` — `OWNER` is the creator, assigned once at `createFamily`, never reassignable, matches the user's "creator" concept exactly. `FamilyAccessService.hasWriteAccess` now grants write access to any member (previously excluded a since-removed `VIEWER`); `requireAdminAccess` was replaced with `requireCreatorAccess` (`OWNER`-only). In `FamilyService`: `addMember` (invite), `listPendingInvitesForFamily`, `cancelInvite`, `addManagedMember`, `listInvitesHistory`, and `updateRelationship` all moved from `requireAdminAccess` to `requireRole` (any member); `removeManagedMember` moved to `requireCreatorAccess`; `removeMember` was rewritten to allow the request when the acting member is either the creator (removing anyone) or removing themselves (leave) — never when a non-creator member targets someone else, never when the target is the `OWNER`. `updateMemberRole` (service method, controller endpoint, and its `UpdateMemberRoleRequest` DTO) was deleted outright — with only an immutable `OWNER` and a default `MEMBER` left, there was nothing left to change a role _to_. `AddMemberRequest` lost its `role` field entirely — every invite is created as `MEMBER`, so the client no longer sends (and the invite sheet no longer asks for) a role at all. `assertAssignableRole` became dead code once `role` left both call sites and was deleted with them.
- **Backend, `medchest` package:** `MedicineService.deleteMedicine` now takes `actingUserId`, looks the medicine up via the existing `findMedicine` helper (so a bad id 404s like every other method in the class, instead of the ad hoc `existsById` check it had), and calls `familyAccessService.requireWriteAccess` before deleting — closing the security hole. `MedicineController` passes `principal.id()` through. `MedicineService.saveMedicinesFromPrescription` changed from `void` to returning `List<Medicine>`, and a duplicate medicine name during AI extraction is now **skipped** instead of thrown — the old behavior threw `BadRequestException`, and because `MedicalDocumentService.uploadDocument` is `@Transactional`, that rolled back the _entire_ upload (the S3 file, the `MedicalDocument` row, and every other successfully-extracted medicine) over one name collision. `MedicalDocumentService.uploadDocument` now wraps the parse-and-save step in a try/catch: success sets `ocrStatus = PROCESSED` (previously hardcoded to `PENDING` forever, regardless of outcome); any exception (unreadable image, invalid AI JSON, etc.) is logged and sets `ocrStatus = FAILED` instead of failing the whole request — the document and file still get saved either way. `MedicalDocumentResponse` gained `extractedMedicineNames: List<String>`, populated on the upload response only (the plain list-documents path always returns `[]`, since there's no stored document→medicine link to recompute it from later).
- **Frontend, family:** `FamilyRole` → `'OWNER' | 'MEMBER'`; `ASSIGNABLE_ROLES` and the invite/edit-role chip pickers are gone. `MyMembership.isAdmin` renamed to `isCreator` (`role === 'OWNER'`). `FamilyScreen.tsx`: the invite button, "Add Managed Member" banner, pending-invites list, and invite-history panel are now visible to any member, not just a resolved admin; the readonly banner is gone entirely; the whole "edit member" bottom sheet (role picker + relation-correction UI) was removed, since there's no role left to edit — every member card now opens the same read-only "view member" sheet, which gained a creator-only "Remove" button (hidden on the creator's own row); "Leave Family" works directly for any non-owner member instead of showing a "needs admin" dead end.
- **Frontend, medicine:** `canEdit` is now unconditionally `true` whenever a membership resolves (previously `membership.isAdmin`, which was stricter than what the backend actually granted even before this change — the backend's `MEMBER` role already had write access to Medicine). `deleteMedicine()` added to `src/features/medicine/api.ts` (`DELETE /medicine/{id}`, singular — the backend's actual route, left as-is) and wired into `MedicineScreen.tsx`'s existing "Remove Medicine" button, which now works in family mode too, not just local-only. `handlePickDocument` now reads the upload response's `extractedMedicineNames`: on a non-empty list it refreshes the medicine list and shows what got auto-added; on `ocrStatus === 'FAILED'` it shows a distinct "couldn't read that prescription" message. The document list now shows a translated `ocrStatus` label instead of the raw enum string, since the value now genuinely changes.
- **Explicitly kept unchanged, per direct confirmation:** stock still auto-decrements by 1 whenever an intake is logged `TAKEN` (`MedicineService.logIntake`), in addition to the pre-existing manual edit via the Stock field on the edit form — the user was asked whether logging an intake should still touch stock given the "manual quantity" framing, and confirmed keeping both.

**Data note.** `FamilyRole`/`FamilyInvite.role` are stored as plain `varchar(20)` (no DB-level enum/check constraint, no Flyway/Liquibase in this project — see `docs/BACKEND_CONTEXT.md`), so shrinking the Java enum does nothing to rows already persisted with the now-removed `ADMIN`/`VIEWER` values — Hibernate throws `IllegalArgumentException: No enum constant ... ADMIN` the moment it tries to read one back (confirmed live against the local dev DB, `family_members`/`family_invites` each had one such row from before this change). Any environment picking up this migration needs `UPDATE family_members SET role = 'MEMBER' WHERE role IN ('ADMIN', 'VIEWER'); UPDATE family_invites SET role = 'MEMBER' WHERE role IN ('ADMIN', 'VIEWER');` run once before the new code can read existing family data.

**Consequences.** All 6 locale files updated in lockstep (verified via a Node script comparing key sets: all six have identical keys) — removed `family.readonly_note`, `family.leave_needs_admin_title/msg`, `family.role_admin`, `family.stat_admins`, `family.sheet_title_edit`, `family.label_role`, `family.save_changes` (the family one; medicine's own `save_changes` key is untouched), `family.relation_correction_helper`, and `medicine.readonly_note`; added `medicine.ocr_status_pending/processed/failed` and `medicine.prescription_processed_title/msg`/`prescription_failed_title/msg`. `Saheli Backend — Auth, Profile & Family.postman_collection.json` updated to match: the invite request body drops `role` (and gained the `relation` field its own saved examples had been missing despite the backend requiring it — a pre-existing staleness fixed as a drive-by since that exact block was already being edited), the "Update Member Role" request was deleted, "Delete medicine"'s description now documents the access check, and the document-upload response examples show `extractedMedicineNames` and a realistic `PROCESSED` status instead of a permanently-`PENDING` one. Not verified live — same caveat as every other Medchest/Family change in this doc — though this session is the first one running against a local instance of the real backend rather than only the collection's saved examples (`src/config.ts` currently points at `http://10.0.2.2:8080/api`, not the deployed host), so a manual smoke test is realistic here in a way it wasn't for D-023 through D-038.
...
this same collection, ahead of this client catching up) describing it as the _true_
...
- Mood data is no longer device-private. D-030 deliberately kept Mind & Mood off the
  family-shared path; that reasoning was about _family sharing_, and still holds — nothing
...
- `types.ts` / `parse.ts` — the four payloads as a discriminated union, and a parser that
  ...
  the _previous day_ anywhere west of Greenwich.
...
- **The channel id must NOT be declared in `AndroidManifest.xml`.** This one was found by
  ...
  agree, and that the app manifest does _not_ re-declare the key.
...
pulled without running `npm install` gets no crash, no dialog, and — the tell — _no
permission prompt at all_, because `NoopPushMessaging.requestPermission()` returns
...
- `OD-4` is now answered for remote push. `M4-T4`'s _local_ dosage reminders are still
  unbuilt, but Notifee is now present and is the tool for them — so that task no longer
  needs a library decision, only the work.

---

## D-060 — Medchest "unexpected error" on add: a backend 500 on the _list_, misreported as a failed _save_
...
**Client bug this exposed, and fixed.** `handleSave` wrapped the write _and_ the follow-up
