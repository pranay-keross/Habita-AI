...
| `POST` | `/api/families/{id}/members` | `{phone, role, relation}` — creates a `PENDING` invite for an **already-registered** user, does not attach them. `role` can't be `OWNER`. `relation` (new 2026-08-13, D-030) is the invitee's relation to the caller, e.g. inviting your mother: `relation: "MOTHER"`. Needs `OWNER`/`ADMIN` |
...
| `POST` | `/api/families/invites/{id}/accept` | **Now requires a body**, `{reciprocalRelation}` (was bodiless before D-030) — the inviter's relation back to the caller, e.g. accepting a `MOTHER` invite: `reciprocalRelation: "SON"` or `"DAUGHTER"`. Creates the `FamilyMember` row (and a `FamilyRelationship` row) with the invite's role/relation, marks invite `ACCEPTED`. Capacity (max 5) re-checked here |
...
| `PATCH` | `/api/families/{id}/relationships/{relationshipId}` | `{relation, reciprocalRelation}` — corrects a relationship after the fact (e.g. invitee picked the wrong reciprocal at accept time), without touching membership. Needs `OWNER`/`ADMIN`. Added 2026-08-13 (D-030) |
...
| `POST` | `/api/families/{id}/managed-members` | `{name, relationship}` (`relationship` is free text, not an enum — unaffected by D-030's `FamilyRelation` enum, which only applies to User-backed members) — attaches immediately, no consent step, since a dependent can't consent. Needs `OWNER`/`ADMIN` |
...
| `POST` | `/api/families/{id}/profiles` | `{name, category, dateOfBirth}` → `{id, name, category, dateOfBirth}`. `category: "CHILD"` 500s ("An unexpected error occurred") in the one captured example — not a clean 400, same category of validation gap as the Profile module's known `preferredLanguage` issue |
| `GET` | `/api/families/{id}/profiles` | List of `FamilyProfile` for the family |
...
| `POST` | `/api/profiles/{familyProfileId}/documents` | multipart: `file` part + `documentType` sent as a JSON-typed text part (same `{string, type: 'application/json'}` shape Profile's `profileRequest` needs, D-013–D-015) — only `"PRESCRIPTION"` appears in the collection's examples. Response: `{id, originalFilename, documentType, uploadedAt, ocrStatus}` |
| `GET` | `/api/profiles/{familyProfileId}/documents` | List of documents for that profile |

**How the client resolved the two remaining gaps (D-035, since deployment is now confirmed) — product choices, not guesses:**

1. **No delete endpoint exists anywhere in this domain** — not for a profile, a medicine, or a document. Rather than leave `MedicineScreen.tsx`'s "Remove Medicine" pointed at nothing, it's now conditionally rendered — present only in local-storage (no-family) mode, absent entirely once the real backend is in play. **If backend work resumes, adding delete endpoints for profile/medicine/document** (or confirming removal is intentionally out of scope for this domain) would let this be restored for real.
2. **Every medicine belongs to a `FamilyProfile`, with no `GET /profiles/me`.** The client now auto-creates/finds a `category: SELF` profile representing the signed-in user, same as `resolveMyMembership()`'s pattern for Family — but `dateOfBirth` had no source anywhere in this client, so a one-time date-of-birth prompt was added, asked only the first time a family with no `SELF` profile tries to add a medicine. **Unconfirmed:** whether the backend actually requires a real `dateOfBirth` or just stores whatever's sent, and whether `SELF` is meant to be one-per-family (this client's assumption) or something else entirely.
...
**Documents, wired 2026-08-13 (`docs/DECISIONS.md` D-037):** `documentType` is fixed client-side to `"PRESCRIPTION"` — the only value that appears anywhere in the collection's examples, so the only one confirmed accepted; the full enum (if there is one) is unconfirmed. `ocrStatus` (`"PENDING"` in the one captured example) is shown raw, not translated, since the full enum is equally unconfirmed — and there is no endpoint anywhere to check when OCR finishes or fetch its result, so `ocrStatus` may just sit at `"PENDING"` forever from this client's point of view. Same no-delete gap as medicines/profiles: uploaded documents cannot be removed.
