...
| `POST` | `/api/profiles/{familyProfileId}/medicines` | `{name, dosage, scheduleTimes: ["HH:MM", ...], stockQuantity, lowStockThreshold}` → adds `{id, lowStock, adherenceRate}` — both computed server-side, not client-derived like this app's current local `calculateAdherence`. `name` and `scheduleTimes` are mandatory; `stockQuantity` and `lowStockThreshold` (default 3) are integer counts. |
| `GET` | `/api/profiles/{familyProfileId}/medicines` | List of medicines for that profile. Returns `{id, name, dosage, scheduleTimes, stockQuantity, lowStockThreshold, lowStock, adherenceRate}`. Note: `scheduleTimes` may be returned as an array or a slot-keyed map (e.g. `{"morning": "08:00", "night": "21:00"}`), `lowStockThreshold`/`stockQuantity` can be null on OCR-created items, and `adherenceRate` may be a number, numeric string, or `"No logs being recorded"`. 404 `"Family profile not found"` on a bad id |
| `PUT` | `/api/medicines/{medicineId}` | Same body shape as create `{name, dosage, scheduleTimes, stockQuantity, lowStockThreshold}` — full replace, not a partial patch. `lowStockThreshold` must be a valid integer (default 3) and not null. |
...
| `DELETE` | `/api/profiles/{familyProfileId}/documents` | Takes a JSON array of document IDs e.g. `["<documentId>"]` → deletes the specified documents. Added to Postman collection 2026-08-21 (D-042). |
...
**Medicine now has a working, secured delete endpoint** (`DELETE /api/medicine/{id}`, see the table above — fixed and wired up D-039, 2026-08-18). **Documents have a delete endpoint** (`DELETE /api/profiles/{familyProfileId}/documents`, body `string[]` of IDs). **Profiles still have no delete endpoint** — that gap is unchanged. **If backend work resumes, adding a delete endpoint for profiles** (or confirming removal is intentionally out of scope) would close the rest of this gap.
...
| `devices` | Push notification device registration | `M4-T4` | `POST /api/devices/register` integrated and live-tested (`src/features/notifications/api.ts`, D-059). **Postman is wrong:** it documents `{phone, code}`, which 500s — the real body is `{fcmToken, platform}` (`DeviceTokenRequestDto`), and the route 403s without a bearer token. Registration is idempotent. Client-side the four alert payloads are parsed, stored, localized and routed; the FCM transport itself is stubbed pending OD-4 |
| `wellness` | Mental Health & CBT Coaching | `M4-T6/T7` | Client fully integrated against the real, live-tested contract (`src/features/wellness/api.ts`, `docs/DECISIONS.md` D-058): check-in CRUD + `limit`-based pagination, CBT chat history/clear, AI uplift, smart recommendation, exercise library + session completion, and `/wellness/summary`. **One backend gap left:** `reason` is absent from `MoodCheckInRequest`, so it is accepted, discarded and always reads back `null`. (`POST /wellness/cbt/chat` 500'd on 2026-08-31 and was fixed server-side the same day) |
...
| `wardrobe` | Wardrobe & Weather-Adaptive Style Mirror | `M7-T3/T4` | Server LIVE (Port 8080, 2026-09-15) — all 30 `/api/style/**` routes in `docs/WARDROBE_API_SPEC.md` (items with S3 photos, wishlist/purchase price, weather, occasions, rule-based recommendation with `mood` + `refinementNote` + ad-hoc `eventType`, saved outfits, style log, collections, trips). Client rewritten server-authoritative in `src/features/style_pantry/` (D-063): typed `api.ts` mappers, one `ReadResult`/`WriteResult` store contract, cache-only offline reads, no seed/mock data, backend error codes surfaced via `errors.ts` |
