...
**Source of truth for what to build next.** Updated 2026-08-13 against branch `initial-static`, after `M2-T1`/`M2-T4`/`M2-T6`–`M2-T8` (`docs/DECISIONS.md` D-013–D-018), the Saheli → Habita AI rebrand (D-019, D-020), real Family & Managed Members backend integration (D-023 — supersedes `M3`'s local model, closes `M2-T9`, and partially closes `M8-T1`/`M8-T3`), a Postman-collection reconciliation pass (D-024 — adds the collection's `invites/history` endpoint, fixes stale filename references across the doc set, flags unfinished `medchest` scaffolding spotted in the collection), and a second reconciliation pass against the same collection (D-030 — the backend added relation tracking to Family members: `relation`/`reciprocalRelation` on invite/accept, a `GET /families/relations` lookup, and a correctable `FamilyRelationship` resource; all now mirrored client-side).
