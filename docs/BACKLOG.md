...
| M4-T5 | Respect the family `medicines` permission when viewing another member's chest | Viewer role cannot edit | ✅ 2026-08-08 — the family member flagged `relation: 'self'` (same convention `FamilyScreen.tsx` uses to mean "the current device's user") is checked on load; if their role is `viewer` or their `medicines` permission is off, the screen renders read-only (no Add button, no edit-on-tap, no "mark taken"), with a localized banner explaining why. Same caveat as `M3-T6`: true per-device "which member is using this app" identity needs the multi-tenant backend from `M8`. **Superseded 2026-08-18 (D-039):** this local `relation`/`viewer`/permission model is long gone (D-023 dropped it for the real Family backend). The real backend never had a per-module permission matrix either, and as of D-039 its role model itself collapsed to creator/member — every family member now gets full add/edit/delete access to Medicine, with no read-only tier at all. |
| M4-T6 | `src/features/wellness/` scaffold: mood entry logging (SRS Mental Health & CBT Coaching) | Mood entries persist to `habita.mood_entries`; tile navigates | ⏳ |
| M4-T7 | Static, localized guided-meditation content list — real AI CBT coaching waits on M8's LLM integration (`M8-T4`) | Content renders and is selectable in all six locales; no network call | 🔒 needs M4-T6 |
| M4-T8 | `src/features/cycle/` scaffold: period-date logging and a local next-cycle prediction (SRS Hormonal Health & Life-Stage Tracking) | Cycle entries persist to `habita.cycle_log`; prediction is a pure, unit-tested function | ⏳ |
| M4-T9 | Life-stage tailored content — fertility planning, postpartum recovery, perimenopause, menopause guidance, static and local | Content selectable by life stage, localized in all six locales | 🔒 needs M4-T8 |
| M4-T10 | Wire `MedicineScreen.tsx`/`medicineStore.ts` to the real Medchest backend (`docs/BACKEND_CONTEXT.md`'s Medchest subsection, `docs/DECISIONS.md` D-032/D-035) — the API layer (`src/features/medicine/api.ts`) already exists | Medicine CRUD, intake logging, and adherence read from the real backend when a family exists, matching Family's D-023 local-to-real transition | ✅ 2026-08-13 — D-035: deployment confirmed live via a direct `curl` check (matching `GET /families`'s known-live `403`-no-token shape); "Remove Medicine" is hidden entirely in real-backend mode since no delete endpoint exists anywhere in the domain. No family still means unchanged local-storage behavior. **2026-08-13 follow-up (D-037):** prescription/document upload also wired — a "Prescriptions & Documents" section (image/PDF/Word picker via a new `@react-native-documents/picker` dependency, `documentType` fixed to `"PRESCRIPTION"`), same `familyProfileId` gate, no local-storage fallback since documents have none. **2026-08-17 follow-up (D-038):** a family can now have multiple medicine profiles, not just one for the caller — the backend added `familyMemberId` linking to Create/List Profile, so the screen gained a profile switcher (chips) and a two-step "Add Profile" sheet (pick an existing family member/dependent, or "Someone Else" for a no-account person, then category + date of birth via D-036's native picker). **2026-08-18 follow-up (D-039):** "Remove Medicine" now works in real-backend mode too — the backend's `DELETE /api/medicine/{id}` existed but had no access control at all (any authenticated user could delete any medicine, cross-family); fixed and wired up client-side. The prescription-upload pipeline's three gaps closed the same day: `ocrStatus` now genuinely transitions `PENDING → PROCESSED/FAILED` instead of sitting at `PENDING` forever, a duplicate medicine name during AI extraction now skips that one medicine instead of rolling back the whole upload, and the upload response's new `extractedMedicineNames` drives an on-screen "medicines added from your prescription" confirmation. Not yet verified end-to-end with a real access token (create profile → add medicine → log intake → upload document) |
