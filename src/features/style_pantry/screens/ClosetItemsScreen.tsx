...
export default function ClosetItemsScreen({ navigation, route }: Props) {
  ...
  const [collectionItemIds, setCollectionItemIds] = useState<string[] | null>(
    null,
  );
  ...
  const filteredItems = items.filter((item) => {
    if (collectionId && collectionItemIds && !collectionItemIds.includes(item.id)) {
      return false;
    }
    if (seasonFilter && item.season !== seasonFilter) return false;
    if (wishlistOnly && !item.isWishlist) return false;
    if (!wishlistOnly && !collectionId && !seasonFilter && item.isWishlist) {
      return false;
    }
    ...
    <View style={styles.root}>
      ...
      <View style={[styles.headerBar, { paddingTop: insets.top + 8 }]}>
        ...
        <Text style={styles.headerTitle} numberOfLines={1}>
          {title || t('style_pantry.dash_title')}
        </Text>
        <Pressable
